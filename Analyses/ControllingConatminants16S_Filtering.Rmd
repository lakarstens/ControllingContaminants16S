...
This RMarkdown file provides the code for evaluating how unrecommended methods - removing all sequences that appear in a blank control and an abundance filter would perfrom for removing contaminants.
...
* true_neg - ASVs correctly identified as mock community ASVs (correctly remianing int the dataset after filtering)
* false_neg - ASVs incorrectly identified as mock community ASVs (unremoved contaminants)
* true_pos - ASVs correctly identified as contaminant ASVs (correctly removing a contaminant ASV from the dataset)
* false_pos - ASVs incorrectly identified as contamiant ASVs (incorrectly removing a mock community read from the dataset)
