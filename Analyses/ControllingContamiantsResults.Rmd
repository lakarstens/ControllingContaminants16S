...
* [Removing contaminant ASVs with filtering methods](./ControllingContaminants16S_filtering.Rmd)
...
Here, we subset the data into
 1. results_asv_long  - This variable has only the ASV abundance and associated data in long format to make it easy to work with ggplot et al.  
 ...
 4. exp_uncor_data - this variable has the results for the original uncorrected data and the expected results if our contaminant removal was perfect. 
...
In addition to calculating classification metrics, we also evaluated the effect of removing the contaminant ASVs on the summary metrics typically used in microbiome studies - alpha diversity metrics such as the number of Observed ASVs, Shannon Diversity, and Inverse Simpson indices. 
...
From the above plot, we can see that leaving contaminants unaccounted for can lead to drastically inflated values of alpha diversity. We also evaluated the alpha diversity measures after removing contaminants (Supplemental Figure 2). 
...
## Composition (Supplemental Figure 3)
