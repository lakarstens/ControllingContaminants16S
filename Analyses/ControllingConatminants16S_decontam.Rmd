...
* true_neg - ASVs correctly identified as mock community ASVs
* false_neg - ASVs incorrectly identified as mock community ASVs (unremoved contaminants)
* true_pos - ASVs correctly identified as contaminant ASVs
* false_pos - ASVs incorrectly identified as contamiant ASVs
```{r }
...
cp_c <- prune_taxa(contam.freq$contaminant, mock_ps)
cp_c <- prune_samples(sample_names(mock_ps), cp_c)
otu_cp_c <-as.data.frame(cp_c@otu_table)
# normalize to relative abundance of initial data
otu_cp_c <- otu_cp_c/sample_sums(mock_ps)

# Fill in the zero filled OTU table with the decontam identified contaminants
con_otus <- otu_temp
con_otus[,colnames(otu_cp_c)] <- otu_cp_c

# identify ASVs correctly identified as mock
true_neg <- rowSums(nc_otus[,colnames(nc_otus) %in% mock_taxa])
# identify ASVs incorrectly identified as mock
false_neg <- rowSums(nc_otus[,!colnames(nc_otus) %in% mock_taxa])

### identify sequences correctly identified as contaminants
true_pos <- rowSums(con_otus[,!colnames(con_otus) %in% mock_taxa])
### identify sequences incorrectly identified as contaminants
false_pos <- rowSums(con_otus[,colnames(con_otus) %in% mock_taxa])
...
# Create color palette
customPalette <- c('#969696','#bdbdbd','#1B9E77', '#E7298A')

# plot
ggplot(long_asv_profile, aes(x = Var2, y = Abundance)) + geom_col(aes(fill = Var1), width = 0.7, position = position_fill())  + scale_fill_manual(values=customPalette) + theme(text = element_text(size=12)) + labs(x = "Dilution Series", y = 'Proportion of Reads')

print('Proportion of contaminants remaining after decontam:')
print(format(round(false_neg*100, 2), nsmall = 2))

print('Proportion of contaminants identified by decontam:')
print(format(round(true_pos/(true_pos+false_neg)*100, 2), nsmall = 2))
